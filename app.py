                                ...
                                try:
                                    ...
                                    # إرسال Ghost packet تلقائيًا لكل الحسابات بعد الحصول على 0500
                                        ...
                                        for _ in range(3):
                                            ...
                                            self.socket_client.send(ghost_pakcet(idT, "tt:nur_sa1lauov", sq, key, iv))
                                            time.sleep(0.5)
    ...
    def guest_token(self, uid, password):
        ...
        for attempt in range(1, 4):
            try:
                ...
                self.set_last_error(f"guest_token_request_error_attempt_{attempt}: {e}")
                print(f"[{self.account_id}] Guest token request failed (attempt {attempt}/3): {e}")
                time.sleep(min(2 * attempt, 5))
            except ValueError as e:
                self.set_last_error(f"guest_token_json_error_attempt_{attempt}: {e}")
                print(f"[{self.account_id}] Guest token JSON parse failed (attempt {attempt}/3): {e}")
                time.sleep(min(2 * attempt, 5))
        ...
        if not NEW_ACCESS_TOKEN or not NEW_OPEN_ID:
            self.set_last_error("guest_token_missing_fields")
            print(f"[{self.account_id}] Guest token response missing required fields")
            return False
...
def execute_command_all():
    ...
    if command.startswith('/nr='):
        ...
        ghost_names = {
            "4648410979": ghost_name,
            "4648411033": ghost_name, 
            "4648411012": ghost_name,
            "4648411039": ghost_name,
    ...
    # Handle /bngx command
        ...
        ghost_names = {
            "4648410979": "tt:nur_sailauov",
            "4648411033": "tg:nur_sailauov",
            "4648411012": "insta:nur_sa1lauov", 
            "4648411039": "vk:nur_sailauov",
...
def custom_nr_command():
    ...
    ghost_names = {
        "4648410979": ghostname,
        "4648411033": ghostname, 
        "4648411012": ghostname,
        "4648411039": ghostname,
